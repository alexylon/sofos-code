...
- **A single command can now run outside the sandbox without leaving workspace mode** (macOS and Linux). When a command genuinely needs network access or to write outside the project, it can run outside the sandbox for that one command after you approve it — either because the assistant asks first or because a command that looks blocked by the sandbox offers to retry. The new `--ask-for-approval` option (short `-a`) and the `/approval` command choose when you are asked: `on-request` (the default, where the assistant may ask), `on-failure` (a sandbox-looking failure offers a retry), or `never`. The current setting appears in the status line when it is not the default. Clearly destructive commands stay refused even when running outside the sandbox is approved.
