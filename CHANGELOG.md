...
- **The assistant can now run more shell commands without stopping to ask** (macOS and Linux). By default, a command the checks would otherwise refuse or prompt for — an unfamiliar command, or a familiar one that only writes its output to a file — now runs confined by the operating system instead: it can only write inside the project folder, it cannot reach the network — not even local services such as the Docker daemon — and it cannot read files you have blocked from reading, even ones it reaches through a wide search rather than naming them directly. Clearly destructive commands, parent-directory access, and hidden subcommands are still refused. Where the Linux sandbox cannot start, an unfamiliar command asks for approval instead of failing. On Windows, workspace mode behaves the same as unrestricted mode for shell commands in this release: unfamiliar commands prompt the user for approval. The restricted-token framework is in the tree as a foundation for a future Windows release; engaging it requires either a non-Cygwin confined shell or a dedicated sandbox user account, both of which are deferred.
- **Confined commands can no longer change the project's setup files** (macOS and Linux). Even though a confined command can write inside the project, the `.git`, `.sofos`, `.agents`, `.claude`, and `.codex` folders stay read-only, so a command cannot plant a Git hook, rewrite Git settings, or edit the rules that decide which commands are allowed. The folders remain readable; change them with the file tools instead of the shell.
