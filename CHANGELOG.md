...
- **Shell commands now run inside an operating-system sandbox in workspace mode** (macOS and Linux). Every command Sofos runs — familiar build tools and interpreters as well as unfamiliar commands — can only write inside the project folder and the temporary directories, cannot reach the network (not even local services such as the Docker daemon), and cannot read files you have blocked from reading, even ones it reaches through a wide search rather than naming them directly. Familiar commands run without interruption and unfamiliar ones run without a prompt, so the assistant keeps working without stopping to ask while staying inside these limits. Clearly destructive commands, parent-directory access, and hidden subcommands are still refused. Because build and network tools are confined too, commands that need to fetch over the network or write outside the project (for example installing dependencies) now fail in workspace mode; switch to `--unrestricted` for those. Where the Linux sandbox cannot start, familiar commands run without the sandbox and unfamiliar ones ask for approval. On Windows, workspace mode behaves the same as unrestricted mode for shell commands in this release: unfamiliar commands prompt the user for approval. The restricted-token framework is in the tree as a foundation for a future Windows release; engaging it requires either a non-Cygwin confined shell or a dedicated sandbox user account, both of which are deferred.
...
- **A single command can now run outside the sandbox without leaving workspace mode** (macOS and Linux). When a command genuinely needs network access or to write outside the project, it can run outside the sandbox for that one command after you approve it — either because the assistant asks first or because a command that looks blocked by the sandbox offers to retry. The new `--ask-for-approval` option (short `-a`) and the `/approval` command choose when you are asked: `on-request` (the default, where the assistant may ask), `on-failure` (a sandbox-looking failure offers a retry), or `never`. The current setting appears in the status line while you are in workspace mode. Clearly destructive commands stay refused even when running outside the sandbox is approved.
