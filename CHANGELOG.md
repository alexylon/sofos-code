...
- **An OpenAI normal stop now carries an explicit `stop_reason`, matching Anthropic.** Earlier code paths that read `if let Some(stop_reason) = ...` treated OpenAI's normal completion as "no stop reason" and behaved differently than Anthropic for the same outcome.
...
- **Long Anthropic streams parse faster.** The line-by-line buffer used to re-allocate every remaining byte after each SSE event, which scaled poorly on long responses. The buffer now drains in place.
...
- **API and system errors no longer fabricate fake assistant messages.** When a request failed mid-turn, sofos was appending an `[API error: ...]` text block tagged as an *assistant* turn. The next request would then show the model what looked like its own prior admission of failure, sometimes triggering confabulated follow-ups. The error context is now folded into the user turn that actually triggered the error, so the model sees a system note attached to the user's own message.
