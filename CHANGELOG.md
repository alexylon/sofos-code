...
- **Shell commands now run inside an operating-system sandbox by default** (macOS and Linux). Every command Sofos runs — familiar build tools and interpreters as well as unfamiliar commands — can only write inside the project folder and the temporary directories, cannot reach the network (not even local services such as the Docker daemon), and cannot read files you have blocked from reading, even ones it reaches through a wide search rather than naming them directly. Familiar commands run without interruption and unfamiliar ones run without a prompt, so the assistant keeps working without stopping to ask while staying inside these limits. Clearly destructive commands, parent-directory access, and hidden subcommands are still refused. Because build and network tools are confined too, commands that need to fetch over the network or write outside the project (for example installing dependencies) now fail under the sandbox; switch to the `unsandboxed` preset for those. Where the Linux sandbox cannot start, familiar commands run without the sandbox and unfamiliar ones ask for approval. On Windows the default is `unsandboxed`: shell commands run unconfined and unfamiliar ones prompt the user for approval. The restricted-token framework is in the tree as a foundation for a future Windows release; engaging it requires either a non-Cygwin confined shell or a dedicated sandbox user account, both of which are deferred.
- **Confined commands cannot tamper with the project's setup folders** (macOS and Linux). The `.sofos`, `.agents`, `.claude`, and `.codex` folders stay read-only, and `.git` stays read-only for any command other than git itself, so a stray command cannot plant a Git hook or rewrite the rules that decide which commands are allowed. Git's own commands can still update `.git`, so switching branches and local git config keep working. The folders remain readable; change the others with the file tools instead of the shell.
- **One `/permissions` command chooses what the assistant may do.** It opens a picker of five presets, from least to most permissive: `read-only` (inspection tools only), `sandboxed-ask` (the default), `sandboxed-retry`, `sandboxed-strict`, and `unsandboxed` (shell commands run without operating-system confinement). You can also type `/permissions <preset>`. The prompt symbol, input cursor, and status line show the current preset — `:` (read-only), `>` (sandboxed), or `#` (unsandboxed). Start-up flags `--readonly` and `--no-sandbox` pick the read-only and unsandboxed presets. Where no sandbox can run, such as Windows, the default is `unsandboxed` and the three `sandboxed-*` presets are shown but disabled in the picker.
- **A single command can run outside the sandbox without leaving the sandboxed mode** (macOS and Linux). When a command genuinely needs network access or to write outside the project, it can run outside the sandbox for that one command after you approve it. The sandboxed presets decide how this is offered: `sandboxed-ask` (the default) has the assistant ask before running such a command, `sandboxed-retry` offers to retry once when a command looks blocked by the sandbox, and `sandboxed-strict` never lifts it. Clearly destructive commands stay refused even when running outside the sandbox is approved.
- **`web_fetch` now asks before reaching a website.** The first time the assistant fetches a URL from a host, Sofos shows that host and asks whether to allow it, with the option to remember the choice. Allowing a host also covers its subdomains: allow it once and it is not asked about again for the rest of the session, or remember it to save `WebFetch(domain:example.com)` to your local config for future sessions too. If a URL redirects to a different host, that host is checked the same way before the redirect is followed, so a redirect cannot quietly reach a site you have not allowed. Declining blocks the fetch. In a non-interactive run a host that is not already allowed is refused, with a hint to add it to the allow list.
- **`web_fetch` refuses internal and cloud-metadata addresses.** A URL whose host is a loopback, private, or link-local address — including the `169.254.169.254` cloud-metadata endpoint that can hand out cloud credentials — `localhost`, or a `.internal` name is now blocked outright, both for the requested URL and for any redirect it follows. This cannot be allowed through the prompt or an allow rule, because the shell sandbox already closes the network to keep confined commands off those addresses and `web_fetch` should not be a way around it. A consequence is that `web_fetch` can no longer reach a local development server; use a shell command for that.
...
- **Shell commands and code search no longer inherit Sofos's API keys.** A command run on your behalf could previously read `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `MORPH_API_KEY` from its environment and print or store them. These keys, together with the dynamic-loader injection variables `LD_PRELOAD`, `LD_AUDIT`, and `DYLD_INSERT_LIBRARIES`, are now removed before a shell command or the code-search tool runs. Every other environment variable is left in place, so build tools keep working.
- **The sandbox is no longer lifted for a single command while a read-block rule is active.** When you have blocked a path from reading (a `Read(...)` entry in the deny list), that block is only reliably enforced while the sandbox is on. Sofos now refuses to run a command outside the sandbox — whether the model requests it or a failed command offers to retry unconfined — until the read-block rule is removed, so a file you blocked cannot be read by a command approved for an unrelated reason.
- **Parent-directory access disguised with quotes or a backslash is now refused.** A traversal written so the shell rebuilds it — a `..` split apart by quotes, or hidden behind a leading backslash — used to slip past the check that blocks `..` in shell commands. Such forms are now normalised the way the shell would and refused just like a plain `../secret`.
- **A path you blocked from writing is now also protected from confined shell commands** (macOS and Linux). A `Write(...)` deny rule was honored by the file tools but not by a sandboxed shell command, so a redirect like `echo x > blocked/path` could still write a path you had blocked. The sandbox now write-protects the denied path inside the project, so both honor the same rule. The path stays readable; only writes are refused.
- **Resuming a session that relaxes the access mode now warns you.** When a resumed session was saved in a less restricted mode than the one in effect — for example resuming an unsandboxed session after launching in read-only — Sofos prints a notice instead of switching silently, so a saved session cannot quietly loosen your current restrictions. Use `/permissions` to change the mode.
- **A read-block rule for a system folder is now enforced instead of being silently dropped** (macOS and Linux). A `Read(...)` deny that targets a path outside your project and home with a wildcard — for example `Read(/etc/ssl/private/*.key)` — used to be dropped from the operating-system sandbox, leaving it only loosely enforced. On macOS it is now compiled into a precise kernel rule that blocks matching reads, including ones a command reaches indirectly. On Linux, where the sandbox cannot match a wildcard, a command is refused while such a rule is active rather than running with the block unenforced.
