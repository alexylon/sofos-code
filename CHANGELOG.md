...
- **A request that Claude Fable 5 or Claude Opus 5 declines is now answered by another model instead of failing.** Those two models run safety classifiers that can refuse a request. Sofos now asks Anthropic to answer a declined request on a substitute model of its choosing, and prints a bright notice naming the model that stepped in, so a false positive on benign work no longer ends the turn. The refusal message still appears when the substitute declines as well.
...
- **One `/permissions` command now chooses the access preset.** The picker offers `read-only`, `sandboxed-ask`, `sandboxed-retry`, `sandboxed-strict`, and `unsandboxed`; `/permissions <preset>` switches directly. The prompt symbol, cursor, and status line show the active preset. `--readonly` starts in read-only mode, and `--no-sandbox` starts in unsandboxed mode. Presets that need an unavailable sandbox are shown but disabled.
- **A single command can run outside the sandbox without changing the session preset.** In `sandboxed-ask`, the assistant may ask before running a command outside the sandbox when it genuinely needs network access or a write outside the project. In `sandboxed-retry`, a command that appears blocked by the sandbox can offer one unsandboxed retry. In `sandboxed-strict`, the sandbox is never lifted. Destructive commands stay refused in every preset.
- **`web_fetch` now asks before reaching a website.** The first request to a host prompts for approval and can be remembered as `WebFetch(domain:example.com)`. Allowing a host also covers its subdomains. Redirects are checked the same way before they are followed, so a redirect cannot silently reach another host.
- **MCP tools now ask before a server's first use.** Because MCP servers run outside the sandbox with open network access, the first time the assistant calls a tool from a server, Sofos shows the server and tool and asks whether to allow it. You can allow it for the current session or remember it as `Mcp(servername)`, which covers every tool from that server; a `deny` rule blocks a server outright. In non-interactive runs, a server that is not already allowed is refused with a hint.
- **`web_fetch` now refuses internal and cloud-metadata addresses.** Loopback, private, and link-local addresses, `localhost`, `.internal` names, and cloud-metadata endpoints such as `169.254.169.254` are blocked for both requested URLs and redirects. These targets cannot be allowed through the prompt or configuration; use a shell command for local development servers.
- **Third-party notices are now included.** Reused code is listed in `THIRD_PARTY_NOTICES` alongside the project license.

### Changed

- **Read-only mode replaces safe-mode naming.** Use `--readonly` or `/permissions read-only` for inspection-only sessions. Direct requests for edit or shell tools are refused in read-only mode, even if an older conversation tries to call them. MCP server configuration now uses `readonly = "read_only"` or `readonly = "allow"`; the old `safe_mode` key is still accepted.
- **Saved sessions now restore the full permission preset.** A session saved in `sandboxed-strict`, `sandboxed-retry`, `sandboxed-ask`, `read-only`, or `unsandboxed` resumes with that preset when it is available on the current machine. If resuming would make the current session less restricted, Sofos prints a warning instead of loosening access silently.
- **Sandboxed sessions now note that the sandbox bounds writes, not reads.** At startup and when you switch to a sandboxed preset, Sofos states that writes outside the project are blocked while out-of-project reads are not confined by default, and points to `Read(...)` deny rules for protecting a path.
- **Sandboxed command failures now explain the likely sandbox limit and the right next step.** Failure notes mention common sandbox causes such as blocked network access, local daemon sockets, protected writes, or mounts, and they match the active preset so the assistant does not suggest a sandbox lift that the preset refuses.
- **`edit_file` now rejects ambiguous single replacements.** If `old_string` appears more than once, the assistant must include more surrounding context or set `replace_all` for an intentional global replacement. Empty `old_string` values are rejected.
...
- **Dangerous Git operations are recognised through Git-specific disguise paths.** Unsafe Git actions are refused when hidden behind global Git options, path-prefixed Git commands, quoting, backslashes, or configuration options that could redirect Git into aliases, included config, pagers, or helper commands.
- **Confined commands cannot rewrite Sofos and agent setup folders.** On macOS and Linux, `.sofos`, `.agents`, `.claude`, and `.codex` stay read-only inside the sandbox, so a shell command cannot quietly change future permissions or agent instructions.
- **Workspace symlinks can no longer skip external-path checks for missing targets.** A shell command that reaches outside the project through a workspace symlink is checked even when the final target does not exist yet.
- **`Read(...)` deny rules now also protect a shell command's program path.** A script or executable blocked from reading cannot be run by naming it as the command.
- **Confined commands can no longer plant Git hooks by disguising the write as Git.** The `.git` directory is writable only for plain Git commands that need to update repository state; redirects, path-prefixed Git commands, and environment-overridden Git commands keep `.git` read-only.
- **Bash `$'...'` quoting can no longer hide blocked commands.** Commands using that quoting form are refused because the shell can decode it into different command text at run time.
- **Shell commands and code search no longer inherit Sofos API keys.** `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `MORPH_API_KEY`, and environment variables that can inject code into child processes are removed before shell commands or ripgrep run.
- **The sandbox is not lifted while a `Read(...)` deny rule is active.** Because arbitrary shell reads are only reliably blocked by the operating-system sandbox, Sofos refuses one-command sandbox lifts and unsandboxed retries until the `Read(...)` deny rule is removed.
- **Parent-directory access disguised with quotes or a backslash is now refused.** Traversal rebuilt by the shell is treated like a plain `..` path and blocked before the command runs.
- **`Write(...)` deny rules now protect confined shell commands too.** A path blocked from writing is write-protected inside the macOS and Linux sandbox, so redirects and other shell writes follow the same deny rules as file tools.
- **Wildcard `Read(...)` deny rules for system folders are enforced instead of ignored.** macOS enforces matching reads in the sandbox. On Linux, where the sandbox cannot express those wildcard rules safely, the command is refused rather than running with the deny rule unenforced.
