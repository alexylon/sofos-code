...
mod tests {
    ...
    fn short_input_returns_unchanged() {
        let (out, count) = truncate_middle_with_token_budget("hello", 100);
        assert_eq!(out, "hello");
    ...
    fn middle_truncation_keeps_head_and_tail() {
        let s: String = (b'a'..=b'z')
            ...
            .collect::<String>()
            .repeat(10);
        let (out, removed) = truncate_middle_with_token_budget(&s, 4);
        assert!(removed.is_some());
        assert!(out.starts_with("ab") || out.starts_with("a"));
        assert!(out.ends_with("xyz") || out.ends_with("yz") || out.ends_with("z"));
    ...
    fn multibyte_boundary_safe() {
        // Cyrillic 'ъ' is 2 bytes; placing it where naive byte slicing would split
        let mut s = "a".repeat(15);
        s.push('ъ');
        s.push_str(" trailing text past the limit to force truncation");
        let (out, _) = truncate_middle_with_token_budget(&s, 4);
    ...
    fn zero_budget_returns_marker_only() {
        let (out, _) = truncate_middle_with_token_budget("abcdef", 0);
        assert!(out.contains("truncated"));
    ...
    fn empty_input_returns_empty() {
        let (out, removed) = truncate_middle_with_token_budget("", 8);
        assert!(out.is_empty());
