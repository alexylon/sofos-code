//! Thin wrapper around `PROC_THREAD_ATTRIBUTE_LIST`. The sandbox spawn
//! uses it to pass an explicit inherited-handle list so the child sees
//! only the stdio pipes we pass and none of the parent's other handles.
