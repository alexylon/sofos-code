...
pub fn confined_invocation(
    ...
    let mut args = linux::bwrap_args(policy);
...
mod linux {
    ...
    /// Build the Bubblewrap arguments: mount the whole filesystem
    /// read-only, then re-bind the writable roots read-write, expose
    /// `/dev` and `/proc`, and close the network unless the policy opens
    /// it. The caller appends `-- <shell> -c <command>`.
    pub fn bwrap_args(policy: &SandboxPolicy) -> Vec<OsString> {
